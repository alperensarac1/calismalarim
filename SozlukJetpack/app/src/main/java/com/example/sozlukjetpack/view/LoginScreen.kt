...
fun LoginScreen(
    ...
    DisposableEffect(Unit) {
        val obs = androidx.lifecycle.Observer<SimpleResponse> { res ->
            isLoading = false
            if (res.success) {
                onLoginSuccess(res.user_id ?: -1, username)
            } else {
                Toast.makeText(context, res.message ?: "Giriş başarısız", Toast.LENGTH_SHORT).show()
            }
        }
        loginResult.observeForever(obs)
        onDispose { loginResult.removeObserver(obs) }
    }

    Scaffold(topBar = { CenterAlignedTopAppBar(title = { Text("Giriş Yap") }) }) { padding ->
        Column(
            Modifier
                .padding(padding)
                .padding(20.dp)
