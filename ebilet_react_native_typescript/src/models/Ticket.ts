import {City, parseCity} from './City';
import {District, parseDistrict} from './District';
import {Event, parseEvent} from './Event';
import {User, parseUser} from './User';
import {Venue, parseVenue} from './Venue';
...
export function parseCheckedBy(json: any): CheckedBy {
    return {
        id: Number(json?.id ?? 0),
...
export function parseTicket(json: any): Ticket {
    ...
    return {
        ...
        checked_by: json?.checked_by ? parseCheckedBy(json.checked_by) : null,
        checkedBy: json?.checked_by ? parseCheckedBy(json.checked_by) : null,
